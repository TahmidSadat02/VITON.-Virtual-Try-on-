...
const WOMEN_CATEGORIES = ['Dresses', 'Tops', 'Skirts', 'Pants', 'Saree', 'Outerwear', 'Knitwear', 'Accessories']
    ...
    <div>
      ...
      <div className="mb-8">
        <div className="flex items-center gap-3 mb-2">
          <div className="px-3 py-1 rounded-sm text-xs font-medium tracking-wider uppercase" style={{ background: 'rgba(244, 114, 182, 0.15)', color: 'rgba(244, 114, 182, 0.9)', border: '1px solid rgba(244, 114, 182, 0.25)' }}>
            Women
          </div>
        </div>
        <h1 className="text-4xl font-light text-white tracking-tight mb-2">Women&apos;s Collection</h1>
        <p className="text-white/50 font-light">Add, edit, or remove women&apos;s clothing & accessories</p>
      </div>
      ...
      <div className="flex flex-col sm:flex-row gap-4 mb-6">
        ...
        <button
          ...
          style={{ background: 'rgba(244, 114, 182, 0.2)', border: '1px solid rgba(244, 114, 182, 0.3)' }}
        >
      ...
      <div className="rounded-sm overflow-hidden" style={{ background: 'rgba(255, 255, 255, 0.06)', border: '1px solid rgba(255, 255, 255, 0.08)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)' }}>
        {filteredDresses.length === 0 ? (
          <div className="py-24 text-center">
            <p className="text-white/50 font-light mb-6">No women&apos;s items found</p>
            <button
              onClick={() => setShowModal(true)}
              className="inline-flex items-center text-sm text-white hover:underline tracking-wide"
            >
              <Plus className="h-4 w-4 mr-2" />
              Add your first women&apos;s item
            </button>
          </div>
        ) : (
                ...
                {filteredDresses.map((dress) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      ...
                      {dress.is_featured && (
                        <span className="text-xs text-pink-400/70 tracking-wide">Featured</span>
                      )}
                    </td>
                    <td className="px-6 py-4">
...
// Women Item Modal Component
function WomenItemModal({
  dress,
  onClose,
  onSuccess,
}: {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const fileName = `women/${Date.now()}-${Math.random().toString(36).substring(2)}.${fileExt}`
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (dress) {
        ...
        if (error) throw error
        alert('✅ Item added to Women\'s collection!')
      }
        ...
        <div className="p-6 flex justify-between items-center" style={{ borderBottom: '2px solid rgba(255,255,255,0.08)' }}>
          <div>
            <h2 className="text-2xl font-light text-white tracking-tight">
              {dress ? 'Edit Women\'s Item' : 'Add Women\'s Item'}
            </h2>
            <div className="flex items-center gap-2 mt-2">
              <span className="px-2 py-0.5 rounded text-[10px] font-medium tracking-wider uppercase" style={{ background: 'rgba(244, 114, 182, 0.15)', color: 'rgba(244, 114, 182, 0.9)', border: '1px solid rgba(244, 114, 182, 0.25)' }}>
                Women&apos;s Section
              </span>
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-5">
                  ...
                  {uploading && (
                    <div className="absolute inset-0 bg-black/70 flex items-center justify-center">
                      <div className="flex flex-col items-center gap-3">
                        <div className="animate-spin rounded-full h-8 w-8 border-2 border-white/20 border-t-pink-400"></div>
          ...
          <div>
            ...
            <textarea rows={3} value={formData.description || ''}
              ...
              style={{ border: '2px solid rgba(255,255,255,0.1)', background: 'rgba(255, 255, 255, 0.06)' }}
              placeholder="Elegant floral midi dress with flowing silhouette..."
            />
          </div>
          ...
          <div className="flex items-center space-x-8 pt-2">
            <label className="flex items-center cursor-pointer">
              <input type="checkbox" checked={formData.is_visible}
                ...
                className="w-4 h-4 text-pink-400 border-2 border-white/20 rounded-sm focus:ring-0 focus:ring-offset-0 bg-transparent"
              />
